...
def upgrade():
    ...
    op.drop_constraint(
        "change_added_tags_tag_name_fkey", "change_added_tags", type_="foreignkey"
    )
    op.drop_constraint(
        "change_removed_tags_tag_name_fkey", "change_removed_tags", type_="foreignkey"
    )
    ...
    session.query(Tag).filter(Tag.is_default_language.is_(False)).filter(
        Tag.name.in_(duplicate_names)
    ...
    op.create_foreign_key(
        "change_added_tags_tag_name_fkey",
        "change_added_tags",
        "tag",
        ["tag_name"],
        ["name"],
    ...
    op.create_foreign_key(
        "change_removed_tags_tag_name_fkey",
        "change_removed_tags",
        "tag",
        ["tag_name"],
        ["name"],
...
def downgrade():
    ...
    op.add_column(
        "sticker_tag",
        sa.Column(
            "tag_is_default_language", sa.BOOLEAN(), autoincrement=False, nullable=True
        ),
    )
    op.drop_constraint("sticker_tag_tag_name_fkey", "sticker_tag", type_="foreignkey")
    op.create_foreign_key(
