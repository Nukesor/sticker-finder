...
def cleanup(session, context):
    ...
    context.tg_chat.send_message(
        "Cleanup finished.", reply_markup=get_main_keyboard(context.user)
    )
...
def refresh_sticker_sets(session, context):
    ...
    sticker_sets = (
        session.query(StickerSet).filter(StickerSet.deleted.is_(False)).count()
    )
...
def refresh_ocr(session, context):
    ...
    tg_chat.send_message(
        "All sticker sets are refreshed.", reply_markup=get_main_keyboard(context.user)
    )
    context.message.delete()
...
def stats(session, context):
    ...
    total_user_count = (
        session.query(User).join(User.inline_queries).group_by(User).count()
    )
    ...
    text_sticker_count = session.query(Sticker).filter(Sticker.text.isnot(None)).count()
    ...
    deluxe_set_count = (
        session.query(StickerSet).filter(StickerSet.deluxe.is_(True)).count()
    )
    nsfw_set_count = session.query(StickerSet).filter(StickerSet.nsfw.is_(True)).count()
    furry_set_count = (
        session.query(StickerSet).filter(StickerSet.furry.is_(True)).count()
    )
    banned_set_count = (
        session.query(StickerSet).filter(StickerSet.banned.is_(True)).count()
    )
    not_english_set_count = (
        session.query(StickerSet).filter(StickerSet.international.is_(True)).count()
    )

    # Inline queries
    total_queries_count = session.query(InlineQuery).count()
    last_day_queries_count = (
...
"""
    context.message.edit_text(stats, reply_markup=get_main_keyboard(context.user))
